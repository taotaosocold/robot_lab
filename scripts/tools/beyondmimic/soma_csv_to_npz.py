...
class ReplayMotionsSceneCfg(InteractiveSceneCfg):
    ...
    robot: ArticulationCfg = CASBOT_02
...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        if self.frame_range is None:
            motion = torch.from_numpy(np.loadtxt(self.motion_file, delimiter=",", skiprows=skip_header))
        else:
            motion = torch.from_numpy(
                np.loadtxt(
                    self.motion_file,
                    delimiter=",",
                    skiprows=skip_header + self.frame_range[0] - 1,
                    max_rows=self.frame_range[1] - self.frame_range[0] + 1,
                )
            )
        motion = motion.to(torch.float32).to(self.device)
        self.motion_base_poss_input = motion[:, 1:4] * 0.01
        roll = motion[:, 4] * (torch.pi / 180.0)
        pitch = motion[:, 5] * (torch.pi / 180.0)
        yaw = motion[:, 6] * (torch.pi / 180.0)
        self.motion_base_rots_input = quat_from_euler_xyz(roll, pitch, yaw)   # 返回 (w, x, y, z)
        self.motion_dof_poss_input = motion[:, 7:] * (torch.pi / 180.0)
