...
class HumanoidEnv:
    ...
    def load_model(self):
        ...
        self.actor_obs_dim = 130
        self.critic_obs_dim = 256

        dummy_obs = TensorDict({
            "policy_obs": torch.zeros(1, self.actor_obs_dim, device=self.device),
            "critic_obs": torch.zeros(1, self.critic_obs_dim, device=self.device)   # critic 也用同样维度，简化处理
        }, batch_size=[1])
        ...
        self.policy_net = ActorCritic(
            ...
            activation="elu",
            init_noise_std=0.2,
            actor_obs_normalization=False,   # 根据你的训练设置调整
...
if __name__ == "__main__":
    ...
    checkpoint = "/home/ubuntu/Desktop/robot_lab/logs/rsl_rl/unitree_g1_FlowMatching_flat/2026-03-24_20-59-10/best_length.pt"
